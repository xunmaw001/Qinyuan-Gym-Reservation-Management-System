<template>
  <div class="addEdit-block">
    ...
    >
      <el-row >
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'" label="活动编号" prop="huodongbianhao">
            <el-input v-model="ruleForm.huodongbianhao" 
                placeholder="活动编号" readonly></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" v-if="ruleForm.huodongbianhao" label="活动编号" prop="huodongbianhao">
              <el-input v-model="ruleForm.huodongbianhao" 
                placeholder="活动编号" readonly></el-input>
          </el-form-item>
        </div>
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="活动标题" prop="huodongbiaoti">
          <el-input v-model="ruleForm.huodongbiaoti" 
              placeholder="活动标题" clearable  :readonly="ro.huodongbiaoti"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="活动标题" prop="huodongbiaoti">
              <el-input v-model="ruleForm.huodongbiaoti" 
...
export default {
  ...
  methods: {
	...
	this.ruleForm.huodongneirong = this.ruleForm.huodongneirong.replace(reg,'../../../springboott94n4/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `huodongricheng/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.huodongrichengCrossAddOrUpdateFlag = false;
						 this.parent.search();
